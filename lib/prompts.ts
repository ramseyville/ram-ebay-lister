...
Return ONLY valid JSON — no markdown, no code fences, no explanation:
{
  ...
  "size": "Size exactly as on tag. Exception: if the tag shows extended sizing as a bare number+X (e.g. \"5X\", \"6X\") rather than with an L suffix, write it as \"5XL\"/\"6XL\" — that's the format eBay's own listings and taxonomy actually use, even when the physical tag omits the L. If you can tell the garment is Big/Tall fit but genuinely cannot read a specific numeric or standard size code from the tag or photos, leave this field EMPTY — never write a purely descriptive placeholder like \"Big Man\" or \"Big and Tall\" here. Those describe fit, not a size code, and eBay's platform will reject them outright as an unrecognized value. Fit descriptions like that belong in condition_notes, not size.",
