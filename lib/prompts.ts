...
Return ONLY valid JSON — no markdown, no code fences, no explanation:
{
  ...
  "category": "mens_top|mens_pants|mens_shorts|mens_jacket|mens_coat|mens_sweater|mens_jeans|mens_shoes|mens_tie|womens_top|womens_pants|womens_jacket|womens_coat|womens_sweater|womens_jeans|womens_dress|womens_skirt|womens_shoes|handbag|wallet|jewelry|scarf|belt|sunglasses|hat|health_beauty|home_decor|kitchenware|book|toy|collectible|hard_goods|other",
  "size": "Size exactly as on tag. Look carefully at every tag photo, including close-ups — the specific numeric size (e.g. neck size on a dress shirt) is usually printed clearly even when a general fit descriptor like \"Big Man\" is also on the tag; read the actual number, don't stop at the descriptor. Exception: if the tag shows extended sizing as a bare number+X (e.g. \"5X\", \"6X\") rather than with an L suffix, write it as \"5XL\"/\"6XL\" — that's the format eBay's own listings and taxonomy actually use, even when the physical tag omits the L. Dress shirts specifically: the tag often prints neck size, sleeve length, AND a fit word together (e.g. \"18 36/37 Tall\") — write ONLY the neck number here (e.g. \"18\"). Never combine the sleeve range or a fit word like \"Tall\"/\"Big\" into this field; eBay's real Size dropdown for shirts is the bare neck number alone, and a combined string gets rejected outright as an unrecognized custom value. The sleeve range belongs in the Sleeve Length item specific, and Tall/Big & Tall is conveyed by Size Type, never written into size itself. Only if a specific numeric or standard size code is truly not visible anywhere in the provided photos after looking carefully, leave this field EMPTY — never write a purely descriptive placeholder like \"Big Man\" or \"Big and Tall\" here. Those describe fit, not a size code. Fit descriptions like that belong in condition_notes, not size.",
